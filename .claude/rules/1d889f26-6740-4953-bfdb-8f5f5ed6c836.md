<rule_activation id="1d889f26-6740-4953-bfdb-8f5f5ed6c836" title="Implement Message Queue Logging for State Management Operations: Logs Include Correlation" applies_to="**/*">
...
- **R-MQL-001** SHOULD: Logs SHOULD include correlation IDs that allow tracing a message through its entire lifecycle across components.
...
# Search for correlation ID usage
...
<enforcement>
Verification is MANDATORY. Claude Code MUST NOT skip or defer verification of message queue logging requirements. Code review checklist, automated static analysis, and integration tests MUST all pass before acceptance. CI pipeline MUST fail if unlogged queue operations are detected. Post-deployment monitoring MUST alert if expected log patterns are missing from production.
</enforcement>