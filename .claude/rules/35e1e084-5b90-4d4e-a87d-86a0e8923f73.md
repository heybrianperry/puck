<rule_activation id="35e1e084-5b90-4d4e-a87d-86a0e8923f73" title="Standardize External API Documentation with Embedded Analytics and Metadata: External Service Integrations" applies_to="**/*">
...
- **R-EXTAPI-001** SHOULD: External service integrations (analytics, error tracking) SHOULD be conditionally loaded based on environment to avoid tracking in development.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. CI/CD pipeline MUST fail if secrets are detected in code. Pull requests MUST be blocked until hardcoded credentials are removed. Security team MUST be notified for manual review of violations. Credentials MUST be rotated immediately if exposed in version control.
</enforcement>