...
These rules are ALWAYS ACTIVE for all React components in apps/demo/config/blocks that expose public APIs, factory functions and component wrappers that transform external data to internal state, modules importing from @/core that bridge application and framework boundaries, and any code path that parses untrusted data (localStorage, URL params, API responses).

### Rules

- **R-CONTRACT-001** MUST: Data crossing from external sources (localStorage, network, user input) MUST be validated against the public contract before transformation to internal contracts.
