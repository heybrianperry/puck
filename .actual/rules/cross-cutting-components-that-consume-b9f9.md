# Isolate Environment Variables for Public Client Configuration in React Components: Components That Consume

These rules are ALWAYS ACTIVE for all client-side React components in Next.js applications that consume environment variables for public API endpoints, feature flags, and client configuration.

### Rules

- **R-ENV-001** SHOULD: Components that consume environment variables SHOULD validate their presence and provide fallback behavior when undefined.

### Verify

```bash
# Check for NEXT_PUBLIC_ prefixed environment variables in client components
grep -r "process\.env\.NEXT_PUBLIC_" apps/docs/components/ | wc -l

# Verify no non-prefixed environment variables are accessed from client components
...
- Environment variables are validated at component initialization with fallback values or error handling
