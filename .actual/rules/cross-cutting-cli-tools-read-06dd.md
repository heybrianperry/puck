# Use process.env for Runtime Configuration in CLI and Component Packages: Cli Tools Read
...
- **R-PROCENV-001** MUST: CLI tools MUST read process.env.npm_config_user_agent to detect the invoking package manager.
...
- **R-PROCENV-003** MUST: React components MUST check process.env.NODE_ENV === 'development' to enable development-only warnings and debugging features.
...
- **R-PROCENV-005** SHOULD: Browser-targeted packages SHOULD ensure bundler configuration includes process.env replacement and test that process.env references are eliminated from production bundles.
...
# Verify no undocumented process.env access patterns
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All process.env reads must include fallback behavior and be documented. CI pipeline must fail if these rules are violated.
</enforcement>