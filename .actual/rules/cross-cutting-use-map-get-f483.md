# Adopt Array.find() and Map.get() for Component State Lookup in React UI Interactions: Use Map Get
...
- **R-MAP-001** MUST: Use Map.get(key) for retrieving cached component measurements, references, and virtualizer handles indexed by component ID or zone compound identifier.
- **R-MAP-002** MUST: Use Array.find() with explicit predicates for type-safe, predicate-based search in collections where identity depends on property comparison (path matching in drag-drop, href matching in stylesheets, value matching in zoom options).
- **R-MAP-003** MUST: Store Map instances in useRef when lookup state must persist across renders without triggering re-renders: const mapRef = useRef(new Map()).
- **R-MAP-004** MUST: Always return cleanup functions from useEffect that call Map.delete() for component-scoped cache entries to prevent memory leaks.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All new React UI interaction components in packages/core MUST comply with R-MAP-001 through R-MAP-007. Code review MUST verify Map.get()/Array.find() pattern compliance. CI pipeline MUST fail if components use object bracket notation for component ID-indexed caches without justification. Memory profiling tests MUST flag components with growing Map instances lacking proper cleanup.
</enforcement>