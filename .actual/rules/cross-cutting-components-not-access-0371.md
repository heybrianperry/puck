...
- **R-ENV-001** MUST_NOT: Components MUST NOT access process.env variables without the NEXT_PUBLIC_ prefix, as these are server-only and will be undefined in the browser.

### Verify

```bash
# Check for NEXT_PUBLIC_ prefixed variables in client components
grep -r "process\.env\.NEXT_PUBLIC_" apps/docs/components/ | wc -l

# Verify no non-prefixed process.env access exists in client components
