# Prefix Public Environment Variables with NEXT_PUBLIC_ for Client-Side Exposure: Components Conditionally Render

These rules are ALWAYS ACTIVE for all Next.js client-side components, pages, and browser-accessible configuration in this project.

### Rules

- **R-NEXTPUB-001** MAY: Components MAY conditionally render based on the presence of public environment variables.
- **R-NEXTPUB-002** MUST: All client-side environment variable references use the NEXT_PUBLIC_ prefix.
- **R-NEXTPUB-003** MUST: No sensitive credential patterns (PASSWORD, SECRET, KEY, TOKEN) appear with NEXT_PUBLIC_ prefix in environment files.
- **R-NEXTPUB-004** SHOULD: Create .env.example file documenting all NEXT_PUBLIC_ variables with descriptions and example values.
- **R-NEXTPUB-005** SHOULD: Add TypeScript declarations for process.env.NEXT_PUBLIC_* to enable autocomplete and type checking.
- **R-NEXTPUB-006** SHOULD: Implement conditional rendering patterns to gracefully handle missing optional public variables.

### Verify

```bash
# Check for client-side process.env usage without NEXT_PUBLIC_ prefix
grep -r 'process\.env\.' apps/docs --include='*.tsx' --include='*.ts' | grep -v 'NEXT_PUBLIC_' | grep -v 'node_modules' || echo 'All client-side env vars properly prefixed'

# Scan for suspicious public secrets
grep -r 'NEXT_PUBLIC_.*\(PASSWORD\|SECRET\|KEY\|TOKEN\)' . --include='.env*' || echo 'No suspicious public secrets found'

# Verify build completes without environment variable warnings
npm run build 2>&1 | grep -i 'environment variable' || echo 'Build completed without env warnings'
```

**Accept when:**
- All client-side environment variable references use NEXT_PUBLIC_ prefix
- No sensitive credential patterns (PASSWORD, SECRET, private API_KEY) appear with NEXT_PUBLIC_ prefix in environment files
- Build process completes successfully with all required public variables defined
- TypeScript type definitions exist for process.env.NEXT_PUBLIC_* variables
- .env.example file documents all public environment variables

<enforcement>
Claude Code MUST NOT skip or defer verification. All rules in this file are mandatory for client-side code and must be validated before accepting changes to environment variable usage or component conditional rendering logic.
</enforcement>