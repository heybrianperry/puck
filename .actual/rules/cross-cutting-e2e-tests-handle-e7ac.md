# Standardize Public API Contract Testing with Jest Mocking and E2E Validation: E2e Tests Handle

These rules are ALWAYS ACTIVE for all files matching the configured scope: public API contracts, state management reducers and actions, browser-based APIs, and E2E utility functions.

### Rules

- **R-E2E-001** SHOULD: E2E tests SHOULD handle iframe boundary transformations when validating coordinate-based APIs like getBox.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All public API contracts must be validated at multiple architectural boundaries. E2E tests must handle iframe boundary transformations for coordinate-based APIs. CI pipeline must execute both Jest unit tests and Puppeteer E2E tests on every pull request. Pull requests without tests for new public API contracts are blocked from merging.
</enforcement>