# Adopt Client-Side Rendering Pattern for External API Integration in Dynamic Content Editors: Client Side Components
...
1. MUST_NOT: Client-side components MUST NOT expose sensitive API credentials or authentication tokens in client-accessible code; authentication MUST be handled through secure server-side proxies

## Policy Block

- MUST_NOT Client-side components MUST NOT expose sensitive API credentials or authentication tokens in client-accessible code; authentication MUST be handled through secure server-side proxies
