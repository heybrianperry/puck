...
1. MUST: Clean up Map entries using delete() when components unmount or items are removed to prevent memory leaks in long-lived virtualized lists

## Policy Block

- MUST Clean up Map entries using delete() when components unmount or items are removed to prevent memory leaks in long-lived virtualized lists
