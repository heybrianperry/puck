...
1. MUST: Node.js processes that perform multi-step file system operations MUST register handlers for SIGINT and SIGTERM signals using process.on() to coordinate graceful shutdown

## Policy Block

- MUST Node.js processes that perform multi-step file system operations MUST register handlers for SIGINT and SIGTERM signals using process.on() to coordinate graceful shutdown
