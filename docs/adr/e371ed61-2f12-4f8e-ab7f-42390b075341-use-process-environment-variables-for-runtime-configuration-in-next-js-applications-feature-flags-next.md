...
1. SHOULD: Feature flags (NEXT_PUBLIC_IS_CANARY) SHOULD be exposed to client-side code when runtime behavior depends on them

## Policy Block

- SHOULD Feature flags (NEXT_PUBLIC_IS_CANARY) SHOULD be exposed to client-side code when runtime behavior depends on them
