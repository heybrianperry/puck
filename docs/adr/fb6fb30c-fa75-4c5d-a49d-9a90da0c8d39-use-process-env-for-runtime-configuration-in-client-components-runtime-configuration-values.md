...
1. MUST: Runtime configuration values that vary between environments MUST be sourced from process.env with the NEXT_PUBLIC_ prefix for client-side access

## Policy Block

- MUST Runtime configuration values that vary between environments MUST be sourced from process.env with the NEXT_PUBLIC_ prefix for client-side access
