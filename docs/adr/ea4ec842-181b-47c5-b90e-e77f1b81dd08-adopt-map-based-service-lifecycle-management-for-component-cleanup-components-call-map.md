...
1. MUST: Components MUST call Map.delete(componentId) in cleanup functions (useEffect return, component unmount) to remove registered resources when the component or its ID changes

## Policy Block

- MUST Components MUST call Map.delete(componentId) in cleanup functions (useEffect return, component unmount) to remove registered resources when the component or its ID changes
