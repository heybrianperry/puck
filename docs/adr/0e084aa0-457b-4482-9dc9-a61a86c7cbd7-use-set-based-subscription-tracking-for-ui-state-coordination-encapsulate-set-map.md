...
1. SHOULD: Encapsulate Set/Map state within useRef hooks (measureRefsRef.current) when state changes should not trigger re-renders but must persist across render cycles

## Policy Block

- SHOULD Encapsulate Set/Map state within useRef hooks (measureRefsRef.current) when state changes should not trigger re-renders but must persist across render cycles
