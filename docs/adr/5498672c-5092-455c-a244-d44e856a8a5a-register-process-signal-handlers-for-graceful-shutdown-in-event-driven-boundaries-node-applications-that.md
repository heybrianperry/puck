...
1. MUST: Node.js applications that perform multi-step file system operations MUST register handlers for SIGINT and SIGTERM signals using process.on() to enable graceful shutdown

## Policy Block

- MUST Node.js applications that perform multi-step file system operations MUST register handlers for SIGINT and SIGTERM signals using process.on() to enable graceful shutdown
